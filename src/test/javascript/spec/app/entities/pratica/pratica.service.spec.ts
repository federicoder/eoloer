import { TestBed, getTestBed } from '@angular/core/testing';
import { HttpClientTestingModule, HttpTestingController } from '@angular/common/http/testing';
import { PraticaService } from 'app/entities/pratica/pratica.service';
import { IPratica, Pratica } from 'app/shared/model/pratica.model';

describe('Service Tests', () => {
  describe('Pratica Service', () => {
    let injector: TestBed;
    let service: PraticaService;
    let httpMock: HttpTestingController;
    let elemDefault: IPratica;
    let expectedResult: IPratica | IPratica[] | boolean | null;

    beforeEach(() => {
      TestBed.configureTestingModule({
        imports: [HttpClientTestingModule],
      });
      expectedResult = null;
      injector = getTestBed();
      service = injector.get(PraticaService);
      httpMock = injector.get(HttpTestingController);

      elemDefault = new Pratica(0, 0, 'AAAAAAA', 'AAAAAAA', 'AAAAAAA', 'AAAAAAA', 'AAAAAAA', 0, 'AAAAAAA', 0, 0, 'AAAAAAA', 'AAAAAAA', 0);
    });

    describe('Service methods', () => {
      it('should find an element', () => {
        const returnedFromService = Object.assign({}, elemDefault);

        service.find(123).subscribe(resp => (expectedResult = resp.body));

        const req = httpMock.expectOne({ method: 'GET' });
        req.flush(returnedFromService);
        expect(expectedResult).toMatchObject(elemDefault);
      });

      it('should create a Pratica', () => {
        const returnedFromService = Object.assign(
          {
            id: 0,
          },
          elemDefault
        );

        const expected = Object.assign({}, returnedFromService);

        service.create(new Pratica()).subscribe(resp => (expectedResult = resp.body));

        const req = httpMock.expectOne({ method: 'POST' });
        req.flush(returnedFromService);
        expect(expectedResult).toMatchObject(expected);
      });

      it('should update a Pratica', () => {
        const returnedFromService = Object.assign(
          {
            idStudioProfessionaleRef: 1,
            numero: 'BBBBBB',
            nome: 'BBBBBB',
            dataApertura: 'BBBBBB',
            dataChiusura: 'BBBBBB',
            dataScadenza: 'BBBBBB',
            stato: 1,
            motivoChiusura: 'BBBBBB',
            idTitolare: 1,
            prcAvanzato: 1,
            version: 'BBBBBB',
            valuta: 'BBBBBB',
            idTemplatePraticaRef: 1,
          },
          elemDefault
        );

        const expected = Object.assign({}, returnedFromService);

        service.update(expected).subscribe(resp => (expectedResult = resp.body));

        const req = httpMock.expectOne({ method: 'PUT' });
        req.flush(returnedFromService);
        expect(expectedResult).toMatchObject(expected);
      });

      it('should return a list of Pratica', () => {
        const returnedFromService = Object.assign(
          {
            idStudioProfessionaleRef: 1,
            numero: 'BBBBBB',
            nome: 'BBBBBB',
            dataApertura: 'BBBBBB',
            dataChiusura: 'BBBBBB',
            dataScadenza: 'BBBBBB',
            stato: 1,
            motivoChiusura: 'BBBBBB',
            idTitolare: 1,
            prcAvanzato: 1,
            version: 'BBBBBB',
            valuta: 'BBBBBB',
            idTemplatePraticaRef: 1,
          },
          elemDefault
        );

        const expected = Object.assign({}, returnedFromService);

        service.query().subscribe(resp => (expectedResult = resp.body));

        const req = httpMock.expectOne({ method: 'GET' });
        req.flush([returnedFromService]);
        httpMock.verify();
        expect(expectedResult).toContainEqual(expected);
      });

      it('should delete a Pratica', () => {
        service.delete(123).subscribe(resp => (expectedResult = resp.ok));

        const req = httpMock.expectOne({ method: 'DELETE' });
        req.flush({ status: 200 });
        expect(expectedResult);
      });
    });

    afterEach(() => {
      httpMock.verify();
    });
  });
});
